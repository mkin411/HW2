...
av <- 1
bm <- 1
if (av>bm) {print("a wins!")} else if (bm>av) {print("b wins!")} else{print("Tie")}
if (av > bm) {
  print("A wins!")
} else if (av < bm) {
  print("B wins!")
...
#dont need the third bm=av, the other option is just to print
#array vs matrix review
happy<-matrix(1:9, 3,3)
happy
#matrix showing three rows and 3 columns
a.ray2<-array(c(1:24), dim=c(3,4,2))
a.ray2
vector1 <- c(5,9,3)
...
data.frame1<-array(c(vector1, vector2), dim= c(3,3,2), dimnames=list(row.names, column.names, matrix.names))
data.frame1
#no need to put the rows, columns and martix in comments, as they are already identified
